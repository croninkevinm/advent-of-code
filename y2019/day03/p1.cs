...
var points = new Dictionary<(int, int), List<int>>();
...
foreach (var point in points)
{
    ...
    if (!point.Value.Contains(1) || !point.Value.Contains(2))
        continue;

    intersectionDistance = Math.Min(
        intersectionDistance,
        Math.Abs(point.Key.Item1) + Math.Abs(point.Key.Item2)
    );
}

System.Console.WriteLine(intersectionDistance);

void TraceWire(IEnumerable<string> moves, int id, Dictionary<(int, int), List<int>> points)
{
    var x = 0;
    var y = 0;
